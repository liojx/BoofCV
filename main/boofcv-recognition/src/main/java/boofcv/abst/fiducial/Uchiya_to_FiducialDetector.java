...
public class Uchiya_to_FiducialDetector<T extends ImageGray<T>> extends FiducialDetectorPnP<T>
	...
	public void detect( T input ) {
		...
		if (out != null) {
			...
			out.printf(" Uchiya: BI %5.1f EL %5.1f ER %5.1f TR %5.1f UP %5.1f\n",
					tracker.getTimeBinary(), tracker.getTimeEllipse(), tracker.getTimeReject(),
					timeTrack, timeUpdate);
		}
	}
	...
	public LlahDocument addMarker( List<Point2D_F64> locations2D ) {
		...
		LlahDocument document = getLlahOperations().createDocument(locations2D);

		// Let the tracker know the number of documents has increased
		tracker.getTracker().updatedLlahDocuments();
		return document;
	}
